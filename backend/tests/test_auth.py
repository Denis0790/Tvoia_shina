...
@pytest.mark.asyncio
async def test_auth_full_cycle(client):
    r = await client.post("/auth/request-code", params={"phone": "+79161234567"})
    assert r.status_code == 200
    code = r.json()["debug_code"]

    r = await client.post("/auth/verify-code", params={"phone": "+79161234567", "code": code})
    assert r.status_code == 200
    assert "access_token" in r.json()
    assert "refresh_token" in r.cookies

    r_bad = await client.post("/auth/verify-code", params={"phone": "+79161234567", "code": "000000"})
